...
const WS_API = {
  ...
  FIELDS: {
    ...
    SENSES: {
      ...
      LIGHT_ATTRS: ["light_radius", "light_dimradius", "light_otherplayers", "light_hassight", "light_angle", "light_losangle", "light_multiplier"],
    },
  },
...
class WildShapeMenu extends WildMenu {
  ...
  showEditCharData(dataRoot) {
      ...
      this.makeList(
        [
          this.makeLabelValue("HP Attribute", settings[WS_API.FIELDS.CHAR_DATA.HP]) +
            this.makeRightButton("Edit", cmdEdit + WS_API.FIELDS.CHAR_DATA.HP + this.SEP + "?{Attribute|" + settings[WS_API.FIELDS.CHAR_DATA.HP] + "}"),
          this.makeLabelValue("SPEED Attribute", settings[WS_API.FIELDS.CHAR_DATA.SPEED]) +
            this.makeRightButton("Edit", cmdEdit + WS_API.FIELDS.CHAR_DATA.SPEED + this.SEP + "?{Attribute|" + settings[WS_API.FIELDS.CHAR_DATA.SPEED] + "}"),
          ...(isNpc
            ? [
                this.makeLabelValue("SENSES", settings[WS_API.FIELDS.CHAR_DATA.SENSES]) +
                  this.makeRightButton("Edit", cmdEdit + WS_API.FIELDS.CHAR_DATA.SENSES + this.SEP + "?{Attribute|" + settings[WS_API.FIELDS.CHAR_DATA.SENSES] + "}"),
              ]
            : []),
        ],
        " padding-left: 10px"
      ...
      this.makeList(
        [
          this.makeLabelValue("BAR 1", settings[WS_API.FIELDS.CHAR_DATA.BAR_1]) +
            this.makeRightButton("Edit", cmdEdit + WS_API.FIELDS.CHAR_DATA.BAR_1 + this.SEP + "?{Attribute|" + settings[WS_API.FIELDS.CHAR_DATA.BAR_1] + "}"),
          this.makeLabelValue("BAR 2", settings[WS_API.FIELDS.CHAR_DATA.BAR_2]) +
            this.makeRightButton("Edit", cmdEdit + WS_API.FIELDS.CHAR_DATA.BAR_2 + this.SEP + "?{Attribute|" + settings[WS_API.FIELDS.CHAR_DATA.BAR_2] + "}"),
          this.makeLabelValue("BAR 3", settings[WS_API.FIELDS.CHAR_DATA.BAR_3]) +
            this.makeRightButton("Edit", cmdEdit + WS_API.FIELDS.CHAR_DATA.BAR_3 + this.SEP + "?{Attribute|" + settings[WS_API.FIELDS.CHAR_DATA.BAR_3] + "}"),
        ],
        " padding-left: 10px"
      ),
    ];
    ...
    let contents = this.makeList(attributesDataList) + this.makeList(tokenDataList) + this.makeList(rollSettingsDataList) + "<hr>" + this.makeButton("Main Menu", this.CMD.CONFIG, " width: 100%");
  ...
  showEditShape(shifterId, shapeId) {
    ...
    const deleteShapeButton = this.makeButton(
      "Delete Shape",
      cmdRemove + "?{Are you sure you want to delete " + shapeId + "?|no|yes}" + this.SEP + WS_API.FIELDS.TARGET.SHAPE + this.SEP + shifterId + this.SEP + shapeId,
      " width: 100%"
    );
  ...
  showEditShifter(shifterId) {
    ...
    _.each(shifterShapes, (value, shapeId) => {
      shapesDataList.push(
        this.makeLabel(shapeId) +
          this.makeRightButton("Del", cmdRemove + "?{Are you sure you want to delete " + shapeId + "?|no|yes}" + this.SEP + WS_API.FIELDS.TARGET.SHAPE + this.SEP + shifterId + this.SEP + shapeId) +
    ...
    const shapeButtons =
      ...
      this.makeButton(
        ...
        cmdImport + WS_API.FIELDS.TARGET.SHAPEFOLDER + this.SEP + shifterId + this.SEP + "?{Folder Name}" + this.SEP + "?{Find in Subfolders?|no|yes}" + this.SEP + "?{Remove Prefix (optional)}",
        " width: 100%"
      ) +
    ...
    _.each(state[WS_API.STATENAME][WS_API.DATA_SHIFTERS], (value, shifterId) => {
      ...
      listItems.push(
        this.makeLabel(shifterId + (shifterSettings[WS_API.FIELDS.ISNPC] ? " <i>(NPC)</i>" : " <i>(PC)</i>")) +
          this.makeRightButton("Del", cmdRemove + "?{Are you sure you want to delete " + shifterId + "?|no|yes}" + this.SEP + WS_API.FIELDS.TARGET.SHIFTER + this.SEP + shifterId) +
  ...
  (function () {
    ...
    const findShifterData = (tokenObj, silent = false) => {
      ...
      if (target) {
        ...
        } else if (!silent) UTILS.chatError("Cannot find ShapeShifter: " + targetId + ", character id: " + target[WS_API.FIELDS.SETTINGS][WS_API.FIELDS.ID]);
        ...
        if (targetData[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE]) {
          ...
          if (isTargetNpc) {
            ...
            if (targetSenses) {
              ...
              if (hasDarkvision) senses.light_radius = visionSense[1];
              ...
              if (visionSense && visionSense.length >= 2) {
                ...
                if (!hasDarkvision) senses.light_radius = senses.light_dimradius;
              }
            }
          }
        }
    ...
    const handleInputEditSenses = (msg, args, config, senses) => {
      ...
      if (field) {
        if (field == WS_API.FIELDS.SENSES.OVERRIDE) senses[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE] = !senses[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE];
        else if (field) {
