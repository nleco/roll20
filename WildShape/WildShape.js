...
const WS_API = {
    ...
    SETTINGS: {
        ...
        SHIFTER_SIZE: 'normal',
        SHAPE_SIZE: 'auto',
        SHAPE_SIZES: ['auto', 'tiny', 'normal', 'large', 'huge', 'gargantuan'],
        SHAPE_SIZES_SCALE: [
            0, //"auto",
            0.5, //"tiny",
            1, //"normal",
...
class WildShapeMenu extends WildMenu {
    ...
    showEditSenses(shifterId = null, shapeId = null) {
        ...
        let sensesDataList = [
            this.makeLabelValue(overrideName, settings[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE], 'false') +
                this.makeRightButton('Toggle', cmdEdit + WS_API.FIELDS.SENSES.OVERRIDE),
        ];

        if (shifterId && !config[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE]) {
            sensesDataList.push(
                this.makeLabel(
                    "NOTE: Current Config Write Senses value is set to false, senses won't be applied",
                    'font-size: 80%; padding-left: 10px; padding-bottom: 10px'
                )
            );
        }
        ...
        _.each(WS_API.FIELDS.SENSES.LIGHT_ATTRS, (attr) => {
            ...
            if (currAttr === false || currAttr === true) attrField = attrField + this.makeRightButton('Toggle', cmdEdit + attr + this.SEP + WS_API.CMD.TOGGLE);
            else attrField = attrField + this.makeRightButton('Edit', cmdEdit + attr + this.SEP + '?{Attribute|' + currAttr + '}');

            sensesDataList.push(attrField);
        });

        let contents = this.makeList(sensesDataList) + '<hr>' + this.makeButton(cmdBackName, cmdBack, ' width: 100%');
    ...
    showEditCharData(dataRoot) {
        ...
        let attributesDataList = [
            ...
            this.makeList(
                [
                    this.makeLabelValue('HP Attribute', settings[WS_API.FIELDS.CHAR_DATA.HP]) +
                        this.makeRightButton(
                            'Edit',
                            cmdEdit + WS_API.FIELDS.CHAR_DATA.HP + this.SEP + '?{Attribute|' + settings[WS_API.FIELDS.CHAR_DATA.HP] + '}'
                        ),
                    this.makeLabelValue('SPEED Attribute', settings[WS_API.FIELDS.CHAR_DATA.SPEED]) +
                        this.makeRightButton(
                            'Edit',
                            cmdEdit + WS_API.FIELDS.CHAR_DATA.SPEED + this.SEP + '?{Attribute|' + settings[WS_API.FIELDS.CHAR_DATA.SPEED] + '}'
                        ),
                    ...(isNpc
                        ? [
                              this.makeLabelValue('SENSES', settings[WS_API.FIELDS.CHAR_DATA.SENSES]) +
                                  this.makeRightButton(
                                      'Edit',
                                      cmdEdit + WS_API.FIELDS.CHAR_DATA.SENSES + this.SEP + '?{Attribute|' + settings[WS_API.FIELDS.CHAR_DATA.SENSES] + '}'
                                  ),
                          ]
                        : []),
                ],
        ...
        let tokenDataList = [
            ...
            this.makeList(
                [
                    this.makeLabelValue('BAR 1', settings[WS_API.FIELDS.CHAR_DATA.BAR_1]) +
                        this.makeRightButton(
                            'Edit',
                            cmdEdit + WS_API.FIELDS.CHAR_DATA.BAR_1 + this.SEP + '?{Attribute|' + settings[WS_API.FIELDS.CHAR_DATA.BAR_1] + '}'
                        ),
                    this.makeLabelValue('BAR 2', settings[WS_API.FIELDS.CHAR_DATA.BAR_2]) +
                        this.makeRightButton(
                            'Edit',
                            cmdEdit + WS_API.FIELDS.CHAR_DATA.BAR_2 + this.SEP + '?{Attribute|' + settings[WS_API.FIELDS.CHAR_DATA.BAR_2] + '}'
                        ),
                    this.makeLabelValue('BAR 3', settings[WS_API.FIELDS.CHAR_DATA.BAR_3]) +
                        this.makeRightButton(
                            'Edit',
                            cmdEdit + WS_API.FIELDS.CHAR_DATA.BAR_3 + this.SEP + '?{Attribute|' + settings[WS_API.FIELDS.CHAR_DATA.BAR_3] + '}'
                        ),
                ],
                ' padding-left: 10px'
            ),
        ...
        let rollSettingsDataList = [
            ...
            this.makeLabel(
                'If set to true it will change the roll settings on the target shape when a ' + charType + ' Shifter transforms into it',
                'font-size: 80%'
            ),
            this.makeList(
                [
                    this.makeLabelValue('Never Whisper', settings[WS_API.FIELDS.CHAR_DATA.FORCEROLL_NEVER_WHISPER], 'false') +
                        this.makeRightButton('Toggle', cmdEdit + WS_API.FIELDS.CHAR_DATA.FORCEROLL_NEVER_WHISPER + cmdToggle),
                    this.makeLabelValue('Toggle Advantage', settings[WS_API.FIELDS.CHAR_DATA.FORCEROLL_TOGGLE_ADVANTAGE], 'false') +
                        this.makeRightButton('Toggle', cmdEdit + WS_API.FIELDS.CHAR_DATA.FORCEROLL_TOGGLE_ADVANTAGE + cmdToggle),
                    this.makeLabelValue('Manual Damage Roll', settings[WS_API.FIELDS.CHAR_DATA.FORCEROLL_MANUAL_DAMAGEROLL], 'false') +
                        this.makeRightButton('Toggle', cmdEdit + WS_API.FIELDS.CHAR_DATA.FORCEROLL_MANUAL_DAMAGEROLL + cmdToggle),
                ],
                ' padding-left: 10px'
            ),
        ];
        ...
        let contents =
            this.makeList(attributesDataList) +
            this.makeList(tokenDataList) +
            this.makeList(rollSettingsDataList) +
            '<hr>' +
            this.makeButton('Main Menu', this.CMD.CONFIG, ' width: 100%');
    ...
    showEditShape(shifterId, shapeId) {
        ...
        let listItems = [
            ...
            this.makeLabelValue('Size', obj[WS_API.FIELDS.SIZE]) +
                this.makeRightButton('Edit', cmdShapeEdit + WS_API.FIELDS.SIZE + this.SEP + '?{Edit Size|' + this['SHAPE_SIZES'] + '}'),
            this.makeLabelValue('Force Senses', obj[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE], 'false') +
                this.makeRightButton('Edit Senses', cmdShapeEdit + WS_API.FIELDS.SENSES.ROOT),
            this.makeLabel('Override the auto/default senses applied', 'font-size: 80%'),
        ];

        const deleteShapeButton = this.makeButton(
            'Delete Shape',
            cmdRemove +
    ...
    showEditShifter(shifterId) {
        ...
        let settingsDataList = [
            ...
            this.makeLabelValue('Token Name', shifterId) +
                this.makeRightButton('Edit', cmdShifterEdit + WS_API.FIELDS.NAME + this.SEP + '&#64;{target|token_name}'),
            this.makeLabel('Token name needs to match to be able to shapeshift', 'font-size: 80%; padding-left: 10px; padding-bottom: 10px'),
            this.makeLabelValue(pcTag + ' Character', shifterSettings[WS_API.FIELDS.CHARACTER]) +
                this.makeRightButton('Edit', cmdShifterEdit + WS_API.FIELDS.CHARACTER + this.SEP + '?{Edit Character|' + shifterPcs + '}'),
            this.makeLabelValue('Size', shifterSettings[WS_API.FIELDS.SIZE]) +
                this.makeRightButton('Edit', cmdShifterEdit + WS_API.FIELDS.SIZE + this.SEP + '?{Edit Size|' + this['SHAPE_SIZES'] + '}'),
            this.makeLabelValue('Is Druid', shifterSettings[WS_API.FIELDS.ISDRUID], 'false') +
                this.makeRightButton('Toggle', cmdShifterEdit + WS_API.FIELDS.ISDRUID),
            this.makeLabel('Is Druid automatically copies over proficiencies and INT/WIS/CHA attributes', 'font-size: 80%'),
            this.makeLabelValue('Force Senses', shifterSettings[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE], 'false') +
                this.makeRightButton('Edit Senses', cmdShifterEdit + WS_API.FIELDS.SENSES.ROOT),
            this.makeLabel('Override the auto/default senses applied', 'font-size: 80%'),
        ];
        ...
        // shapes section
        let shapesDataList = [this.makeLabel("<p style='font-size: 120%'><b>Shapes:</b></p>")];

        _.each(shifterShapes, (value, shapeId) => {
            shapesDataList.push(
                this.makeLabel(shapeId) +
                    this.makeRightButton(
                        'Del',
                        cmdRemove +
                            '?{Are you sure you want to delete ' +
                    ...
                    this.makeRightButton('Edit', cmdShapeEdit + shifterId + this.SEP + shapeId)
            );
        });
        ...
        const shapeButtons =
            ...
            this.makeButton('Add NPC', cmdShapeAdd + shifterId + this.SEP + '?{Target Shape|' + npcs + '}', 'width: 100%') +
            '<td>' +
            this.makeButton('Add PC', cmdShapeAdd + shifterId + this.SEP + '?{Target Shape|' + pcs + '}', 'width: 100%') +
            '</table>' +
            this.makeButton(
                'Import Shapes from Folder',
                cmdImport +
                    WS_API.FIELDS.TARGET.SHAPEFOLDER +
                    this.SEP +
                    shifterId +
                    this.SEP +
                    '?{Folder Name}' +
                    this.SEP +
                    '?{Find in Subfolders?|no|yes}' +
                    this.SEP +
                    '?{Remove Prefix (optional)}',
                ' width: 100%'
            ) +
            this.makeLabelComment(
                'Adding a single shape may take a several seconds (around 4s), importing several shapes from a folder may take a while (e.g. 15 shapes should take around 1 minute); please wait until you see the results in the chat'
            );

        const deleteShapesButton = this.makeButton(
            'Delete All Shapes',
            cmdRemove + '?{Are you sure you want to delete all shapes?|no|yes}' + this.SEP + WS_API.FIELDS.TARGET.SHAPE + this.SEP + shifterId,
            ' width: 100%'
        );
        ...
        const deleteShifterButton = this.makeButton(
            'Delete Shifter: ' + shifterId,
            cmdRemove + '?{Are you sure you want to delete the shifter?|no|yes}' + this.SEP + WS_API.FIELDS.TARGET.SHIFTER + this.SEP + shifterId,
            ' width: 100%'
        );
        ...
        let contents =
            this.makeList(settingsDataList) +
            '<hr>' +
            this.makeList(shapesDataList) +
            shapeButtons /*+ importShapesButton + exportShapesButton + exportShifterButton*/ +
            '<hr>' +
            deleteShapesButton +
            deleteShifterButton +
            '<hr>' +
    ...
    showShifters() {
        ...
        _.each(state[WS_API.STATENAME][WS_API.DATA_SHIFTERS], (value, shifterId) => {
            ...
            listItems.push(
                this.makeLabel(shifterId + (shifterSettings[WS_API.FIELDS.ISNPC] ? ' <i>(NPC)</i>' : ' <i>(PC)</i>')) +
                    this.makeRightButton(
                        'Del',
                        cmdRemove +
                            '?{Are you sure you want to delete ' +
                            ...
                            '?|no|yes}' +
                            this.SEP +
                            WS_API.FIELDS.TARGET.SHIFTER +
                            this.SEP +
                            shifterId
                    ) +
                    this.makeRightButton('Edit', cmdShifterEdit + shifterId)
            );
        });
    ...
    showConfigMenu(newVersion) {
        ...
        let settingsList = [
            ...
            this.makeLabelValue('Commands Separator', this.SEP) +
                this.makeRightButton('Edit', cmdConfigEdit + WS_API.FIELDS.SEP + this.SEP + '?{New Separator}'),
            ...
            this.makeLabelValue('<br>WildShape Resource', config[WS_API.FIELDS.DRUID_WS_RES]) +
                this.makeRightButton('Edit', cmdConfigEdit + WS_API.FIELDS.DRUID_WS_RES + this.SEP + '?{Edit|' + config[WS_API.FIELDS.DRUID_WS_RES] + '}'),
            this.makeLabel('Automatically check and decrease resource for Druids (case insensitive)', 'font-size: 80%; padding-left: 10px'),

            this.makeLabelValue('Mute Shift Messages', config[WS_API.FIELDS.MUTE_SHIFT], 'false') +
                this.makeRightButton('Toggle', cmdConfigEdit + WS_API.FIELDS.MUTE_SHIFT),
            this.makeLabel('Mute messages sent to players when shapeshifting', 'font-size: 80%; padding-left: 10px'),

            this.makeLabel('PC Settings') +
                this.makeRightButton('Edit', cmdConfigEdit + WS_API.FIELDS.TARGET.CHAR_DATA + this.SEP + WS_API.FIELDS.CHAR_DATA.PC_ROOT),
            this.makeLabel('Global settings (attributes, rolls, etc.) for PC sheets', 'font-size: 80%; padding-left: 10px'),

            this.makeLabel('NPC  Settings') +
                this.makeRightButton('Edit', cmdConfigEdit + WS_API.FIELDS.TARGET.CHAR_DATA + this.SEP + WS_API.FIELDS.CHAR_DATA.NPC_ROOT),
            this.makeLabel('Global settings (attributes, rolls, etc.) for NPC sheets', 'font-size: 80%; padding-left: 10px'),

            // senses settings
            this.makeLabelValue('Write Senses', config[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE], 'false') +
                this.makeRightButton('Edit', cmdConfigEdit + WS_API.FIELDS.SENSES.ROOT),
            this.makeLabel('Write senses to token, defaults if data cannot be found', 'font-size: 80%; padding-left: 10px'),

            this.makeLabelValue('Enable Debug Messages', config[WS_API.FIELDS.ENABLE_DEBUG], 'false') +
                this.makeRightButton('Toggle', cmdConfigEdit + WS_API.FIELDS.ENABLE_DEBUG),
        ...
        const resetButton = this.makeButton(
            'Reset',
            this.CMD.CONFIG_RESET + this.SEP + '?{Are you sure you want to reset all configs?|no|yes}',
            ' width: 100%'
        );

        let title_text = WS_API.NAME + ' v' + WS_API.VERSION + (newVersion ? ': New Version Setup' : ': Config');
        let contents = showShiftersButton + '<hr>' + this.makeList(settingsList) + '<hr>' + resetButton;
...
var WildShape =
    ...
    (function () {
        ...
        const getTokenBarData = (shifterSettings, targetCharacterId, isTargetDefault, isTargetNpc, resetShiftHP=true) => {
            const config = state[WS_API.STATENAME][WS_API.DATA_CONFIG];
        ...
        const findShifterData = (tokenObj, silent = false) => {
            ...
            } else if (!silent) UTILS.chatError('Cannot find ShapeShifter: ' + targetId);
        ...
        async function getTargetCharacterData(shiftData, isTargetNpc, isTargetDefault, resetShiftHP=true) {
            ...
            if (!targetImg) {
                UTILS.debugFlush();
                UTILS.chatErrorToPlayer(
                    shiftData.who,
                    'cannot use image with marketplace link, the image needs to be re-uploaded into the library and set on the target character as either token or avatar image; character id = ' +
                        shiftData.targetCharacterId
                );
                return null;
            } else {
            ...
            if (shifterSettings[WS_API.FIELDS.ISNPC] && !isTargetDefault && shifterSettings[WS_API.FIELDS.CURRENT_SHAPE] == WS_API.SETTINGS.BASE_SHAPE) {
                ...
                if (hpAttr == npcDataRoot[WS_API.FIELDS.CHAR_DATA.BAR_1]) hpBarValue = 'bar1_value';
                else if (hpAttr == npcDataRoot[WS_API.FIELDS.CHAR_DATA.BAR_2]) hpBarValue = 'bar2_value';
                else if (hpAttr == npcDataRoot[WS_API.FIELDS.CHAR_DATA.BAR_3]) hpBarValue = 'bar3_value';
                ...
                if (hpBarValue) {
                    ...
                    UTILS.debugFlush();
                    UTILS.chatError(
                        'cannot save current HP, no bar value found connected to ' +
                            hpAttr +
                            '; this is required for NPC shifters, please make sure the hp attribute is set on one of the bars for the NPC settings in the main config'
                    );
                    return null;
                }
            }
                ...
                if (targetSize == 0) {
                    if (isTargetNpc) {
                        ...
                        await UTILS.getDefaultTokenSize(shiftData.targetCharacter).then((ret) => {
                            if (ret.width > 0) tokenWidth = ret.width;
                            if (ret.height > 0) tokenHeight = ret.height;
                        });
                    }
                }
            ...
            data.barData = getTokenBarData(shifterSettings, shiftData.targetCharacterId, isTargetDefault, isTargetNpc, resetShiftHP);
            ...
            if (config[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE]) {
                ...
                if (targetData[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE]) {
                    ...
                    if (isTargetNpc) {
                        ...
                        let race = getAttrByName(shiftData.targetCharacterId, 'race', 'current');

                        switch (race) {
                            case 'Dwarf':
                                senses.has_night_vision = true;
                                senses.night_vision_distance = 60;
                                break;
                        }
                    }
                }
        ...
        */
            ...
            if (shapeStatAttr) {
                ...
                UTILS.debugChat(
                    'stat ' +
                        profData.shapeStatName +
                        ': ' +
                        profData.shapeStatValue.toString() +
                        ', npc pb: ' +
                        statPb +
                        ', npc mod: ' +
                        profData.shapeStatMod +
                        ', druid pb: ' +
                        (isProficient ? profData.druidPb : 0)
                );
                ...
                if (newShapeStatValue != oldShapeStatValue || shapeStatAttr.get('current') == '') {
                    if (shapeStatAttr.get('current') == '') UTILS.debugChat('SETTING ' + profData.shapeStatName + ': to ' + newShapeStatValue);
                    else UTILS.debugChat('CHANGING ' + profData.shapeStatName + ': from ' + oldShapeStatValue + ' to ' + newShapeStatValue);
                    ...
                    UTILS.setAttribute(
                        profData.shapeId,
                        profData.shapeStatName + WS_API.SETTINGS.STATS.SUFFIX.BASE,
                        (newShapeStatValue > 0 ? '+' : '') + newShapeStatValue.toString()
                    );
                }
        ...
        async function doShapeShift(shiftData, resetShiftHP=true) {
            ...
            if (shiftData.targetShape) {
                ...
                if (!shiftData.targetCharacter) {
                    UTILS.chatErrorToPlayer(
                        shiftData.who,
                        'Cannot find target character = ' + shiftData.targetShape[WS_API.FIELDS.CHARACTER] + ' with id = ' + shiftData.targetCharacterId
                    );
                    return false;
                }
            ...
            await getTargetCharacterData(shiftData, isTargetNpc, isTargetDefault, resetShiftHP).then((ret) => {
                targetData = ret;
            });
            ...
            if (tokenPageData) {
                ...
                if (snapping_increment && snapping_increment > 0) tokenPageScale = snapping_increment;
            }
        ...
        async function addShapeToShifter(config, shifter, shapeCharacter, newCharacterName, shapeId = null, doSort = true) {
            ...
            if (!shapeId || typeof shapeId !== 'string' || shapeId.length == 0) shapeId = shapeName;
            ...
            await UTILS.duplicateCharacter(shapeCharacter, newCharacterName).then(function (newShapeCharacter) {
                shapeCharacter = newShapeCharacter;
            });
                    ...
                    if (obj.targetShapeName !== obj.shifter[WS_API.FIELDS.SETTINGS][WS_API.FIELDS.CURRENT_SHAPE]) {
                        ...
                        doShapeShift(obj).then((ret) => {
                            if (ret && !config[WS_API.FIELDS.MUTE_SHIFT]) {
                                if (obj.targetShape) UTILS.chatAs(obj.shifterCharacter.get('id'), 'Transforming into ' + shapeName, null, null);
                                else UTILS.chatAs(obj.shifterCharacter.get('id'), 'Transforming back into ' + obj.shifterId, null, null);
                            }
                        });
                    } else {
        ...
        async function handleInputAddShape(msg, args, config) {
            ...
            if (shifter) {
                ...
                if (shapeObj) {
                    ...
                    await addShapeToShifter(config, shifter, shapeObj, shapeCharacterName).then((ret) => {
                        if (ret) {
                            MENU.showEditShifter(shifterKey);
                            UTILS.chat('New shape added: ' + shapeName);
                        }
                    });
                } else {
                    UTILS.chatError('Cannot find character [' + shapeName + '] in the journal');
                    UTILS.chatError(
                        "If you see a list in the error above please make sure the last character in the list doesn't have any double quotes \" in the name, you can replace those with single quotes ' "
                    );
                }
            } else {
        ...
        const handleInputEditSenses = (msg, args, config, senses) => {
            ...
            if (field) {
                if (field == WS_API.FIELDS.SENSES.OVERRIDE)
                    ...
                    if (newValue) {
                        if (newValue == WS_API.CMD.TOGGLE) senses[WS_API.FIELDS.SENSES.ROOT][field] = !senses[WS_API.FIELDS.SENSES.ROOT][field];
                        else senses[WS_API.FIELDS.SENSES.ROOT][field] = newValue;
                    }
                }
            }
            ...
            if (shifter) {
                ...
                if (field) {
                    if (field == WS_API.FIELDS.SENSES.ROOT) {
                        ...
                        if (isValueSet) MENU.showEditShifter(shifterKey);
                    }
                } else MENU.showEditShifter(shifterKey);
            } else {
        ...
        async function handleInputImportShapeFolder(msg, args, config) {
            ...
            if (shifter) {
                ...
                if (removePrefix == '') removePrefix = null;
                ...
                if (folderShapes) {
                    if (WS_API.DEBUG) {
                        ...
                        _.each(folderShapes, function (shape) {
                            debugShapeNames += shape.get('name') + ', ';
                        });
                    ...
                    for (let shapeIndex = folderShapes.length - 1; shapeIndex >= 0; --shapeIndex) {
                        ...
                        if (shapeObj) {
                            ...
                            await addShapeToShifter(config, shifter, shapeObj, newName, shapeId, false).then((ret) => {
                                if (ret) importedShapes += 1;
                            });
                        }
                    }
                            ...
                            case WS_API.CMD.CONFIG:
                                {
                                    switch (args.shift()) {
                                        case WS_API.CMD.ADD:
                                            {
                                                switch (args.shift()) {
                                                    case WS_API.FIELDS.TARGET.SHIFTER:
                                                        handleInputAddShifter(msg, args, config);
                                                        break;
                                                    case WS_API.FIELDS.TARGET.SHAPE:
                                                        handleInputAddShape(msg, args, config);
                                                        break;
                                                }
                                            }
                                            break;
                                        ...
                                        case WS_API.CMD.REMOVE:
                                            {
                                                if (args.shift() == 'no') return;

                                                switch (args.shift()) {
                                                    case WS_API.FIELDS.TARGET.SHIFTER:
                                                        handleInputRemoveShifter(msg, args, config);
                                                        break;
                                                    case WS_API.FIELDS.TARGET.SHAPE:
                                                        handleInputRemoveShape(msg, args, config);
                                                        break;
                                                }
                                            }
                                            break;
                                        ...
                                        case WS_API.CMD.EDIT:
                                            {
                                                switch (args.shift()) {
                                                    case WS_API.FIELDS.TARGET.SHIFTER:
                                                        handleInputEditShifter(msg, args, config);
                                                        break;
                                                    case WS_API.FIELDS.TARGET.SHAPE:
                                                        handleInputEditShape(msg, args, config);
                                                        break;
                                                    case WS_API.FIELDS.TARGET.CONFIG:
                                                        handleInputEditConfig(msg, args, config);
                                                        break;
                                                }
                                            }
                                            break;
                                        ...
                                        case WS_API.CMD.IMPORT:
                                            {
                                                switch (args.shift()) {
                                                    case WS_API.FIELDS.TARGET.SHAPEFOLDER:
                                                        handleInputImportShapeFolder(msg, args, config);
                                                        break;
                                                }
                                            }
                                            break;
        ...
        const handleAddToken = (token) => {
            ...
            if (obj) {
                ...
                if (obj.targetShapeName != WS_API.SETTINGS.BASE_SHAPE) {
                    ...
                    doShapeShift(obj, false).then((ret) => {
                        if (ret) {
                    ...
                    setTokenBarValues(
                        token,
                        getTokenBarData(shifterSettings, obj.shifterCharacterId, true, shifterSettings[WS_API.FIELDS.ISNPC]),
                        obj.shifterControlledby
                    );
                }
            }
        };
        ...
        const upgradeVersion = () => {
            ...
            if (UTILS.compareVersion(currentVersion, '1.0.5') < 0) {
                ...
                if (config.SEP == '--') config.SEP = newConfig.SEP;
            }
            ...
            if (UTILS.compareVersion(currentVersion, '1.0.7') < 0) {
                config[WS_API.FIELDS.CHAR_DATA.NPC_ROOT][WS_API.DEPRECATED.NPC_DATA.SENSES] =
                    newConfig[WS_API.FIELDS.CHAR_DATA.NPC_ROOT][WS_API.DEPRECATED.NPC_DATA.SENSES];
            }
        ...
        async function setDefaults(reset) {
            ...
            if (!apiState || typeof apiState !== 'object' || reset) {
                if (state[WS_API.STATENAME]) {
                    _.each(state[WS_API.STATENAME][WS_API.DATA_SHIFTERS], (shifter) => {
                        _.each(shifter[WS_API.FIELDS.SHAPES], (shape) => {
                            removeShape(shape);
                        });
                    });
            ...
            if (reset || oldVersionDetected) {
                ...
                if (oldVersionDetected) {
                    _.each(WS_API.CHANGELOG, function (changes, version) {
                        if (UTILS.compareVersion(oldVersionDetected, version) < 0) UTILS.chat('Updated to ' + version + ': ' + changes);
                    });
        ...
        async function onAttributeChange(obj, prev) {
            ...
            let c = findObjs({ characterid: character_id, name: 'race'});
            // log('----------------');
            // log(obj);
            // log('----------------');
            // log('characte rid: ' + character_id);
            // log('----------------');
            // log(c);
            // log('----------------');
        ...
        async function start() {
            ...
            if (!UTILS.VERSION || UTILS.compareVersion(UTILS.VERSION, WS_API.REQUIRED_HELPER_VERSION) < 0) {
                UTILS.chatError(
                    'This API version ' +
                        WS_API.VERSION +
                        ' requires WildUtils version ' +
                        WS_API.REQUIRED_HELPER_VERSION +
                        ', please update your WildHelpers script'
                );
                return;
            }
